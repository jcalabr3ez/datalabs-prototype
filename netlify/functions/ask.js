...
const ENGINE_SCHEMA = {
  ...
  properties: {
    ...
    highlight: { anyOf: [{ type: 'string' }, { type: 'null' }], description: 'when the answer focuses on one entity: for DL-03 a mode code (HR, MB, CR, LR, RB, FB, DR); for DL-02 the exact county name as written in county_premiums; for DL-01 or DL-04 the exact two-letter jurisdiction code (for example CA, MA, DC, HI); for DL-05 a board id slug (for example state, mtrs, springfield); else null' },
