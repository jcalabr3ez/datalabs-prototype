...
module.exports = [
  {
    ...
    routerScope: 'Covers MBTA ridership (unlinked passenger trips) by mode and month through the dataset as_of month, recovery vs 2019, service, operating cost per trip, and farebox recovery rates. Does NOT cover: safety, reliability or on-time performance, the capital budget, debt, the fare PRICES riders are charged (ticket and pass prices), or other transit agencies.',
    dataset: DL12,
    modelSlice: function (d) { return d; },
    charts: ['monthly_trend', 'recovery_by_mode', 'cost_per_trip', 'farebox'],
    views: null,
    viewDefault: null,
    ...
    rules: 'DL-12 rules. Every figure cites its source in parentheses: ridership figures cite (SRC-301); cost per trip and farebox figures cite (LEG-MBTA-01). Values you compute say derived, e.g. (derived vs same month 2019, SRC-301). Prefer the precomputed values in the derived block over your own arithmetic. Data runs through the dataset as_of month. If and only if the answer uses cost per trip or farebox figures, end the text with: "Cost figures are pending verification." Never call ridership figures prototype or pending; they are verified against FTA NTD. Cost per trip, farebox recovery rates, and ridership are all answerable; never mark them unanswerable. Chart selection: monthly_trend = ridership over time or recovery overall; recovery_by_mode = comparing modes vs 2019; cost_per_trip = cost to provide; farebox = share riders pay; none = no view fits.',
    link: function (p) { return p.chart && p.chart !== 'none' ? '/mbta/#view-' + p.chart : '/mbta/'; },
    src: function (d) { return 'FTA NTD (SRC-301), through ' + d.as_of + '. Ridership verified against the National Transit Database; cost per trip and farebox figures are legacy (LEG-MBTA-01), pending verification.'; }
  },
  {
    id: 'DL-10',
  ...
  },
  {
    ...
    modelSlice: function (d) {
      ...
      return {
        ...
        scope: d.scope, views: d.views, meta: d.meta, derived: d.derived, states: states
      };
    },
    charts: null,
