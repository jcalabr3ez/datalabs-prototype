...
def portfolio_insights(ledgers_by_id):
    """A short set of the most interesting published comparisons across tools."""
    out = []
    dl06 = ledgers_by_id.get("DL-06") or {}
    for fig in figs_dl06(dl06):
        if fig["id"] == "ch74-seats":
            fig = dict(fig)
            fig["url"] = "/ma-k12/"
            fig["span"] = 1
            out.append(fig)
            break
    dl07 = ledgers_by_id.get("DL-07") or {}
    for fig in figs_dl07(dl07):
        if fig["id"] == "naep-2024":
            fig = dict(fig)
            fig["url"] = "/national-k12/"
            fig["span"] = 1
            out.append(fig)
            break
    latest6 = dl06.get("latest") or {}
    ma_ppe = (latest6.get("ma") or {}).get("v")
    us_ppe = (latest6.get("us") or {}).get("v")
    ma_rank = (latest6.get("ma") or {}).get("rank")
    dl06_ppe = from_latest(
        dl06, "ppe-compare",
        title="Current expenditures per pupil, FY 2024",
        lede=(
            f"Massachusetts ${ma_ppe:,}, rank {ma_rank} of 51, versus a "
            f"U.S. average of ${us_ppe:,}."
        ) if ma_ppe and us_ppe else None,
        note="NCES NPEFS FY 2024. Open Massachusetts K-12 for the full ranking.",
    )
    if dl06_ppe:
        dl06_ppe["url"] = "/ma-k12/"
        out.append(dl06_ppe)
    dl14 = ledgers_by_id.get("DL-14") or {}
    wage_snap = _sec(dl14).get("qcew_avg_weekly_wage_2025q4") or {}
    wage = from_snap(
        wage_snap, "qcew-wage",
        title="Average weekly wage, 2025 Q4",
        lede=(
            f"Massachusetts ${int(wage_snap['ma']['v']):,} a week, "
            f"rank {wage_snap['ma']['rank']} of {wage_snap['ma']['n']}."
        ) if wage_snap.get("ma") else None,
    )
    if wage:
        wage["url"] = "/labor-market/"
        out.append(wage)
    dl16 = ledgers_by_id.get("DL-16") or {}
    for fig in figs_dl16(dl16):
        if fig["id"] == "cs-boston":
            fig = dict(fig)
            fig["url"] = "/housing-market/"
            fig["span"] = 1
            out.append(fig)
            break
        if fig["id"] == "fhfa-hpi" and not any(f["id"] == "cs-boston" for f in out):
            fig = dict(fig)
            fig["url"] = "/housing-market/"
            out.append(fig)
    dl15 = ledgers_by_id.get("DL-15") or {}
    for fig in figs_dl15(dl15):
        if fig["id"] == "pcpi":
            fig = dict(fig)
            fig["url"] = "/gdp-profiles/"
            out.append(fig)
            break
    return out[:6]
